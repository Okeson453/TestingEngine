...
function sleep(handle: WorkerHandle, ms: number): Promise<void> {
  return new Promise((res) => {
    ...
    // CRITICAL: only unref in embedded/dev mode (no DATABASE_URL).
    // In the standalone Railway worker the poll timer is the primary
    // keep-alive. Unref'ing it lets Node exit as soon as the first sleep
    // starts — which is why the engine appeared offline with no further logs.
  ...
  try {
    while (handle.running) {
      try {
        ...
        // stdout so Railway captures activity even when the structured logger is quiet
        console.log(
