...
if (typeof window === "undefined") {
  ...
  // Process-lifetime diagnostic: keep firing while the process runs, but do
  // not block orderly exit (unref) — otherwise `node --test` hangs after the
  // last test and graceful shutdown is delayed in scripts importing db.ts.
