...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    // Configure pool for shared/pooler limits (e.g. Layerbase / pgBouncer).
    // Default pg Pool max=10 easily exhausts max_client_conn on small poolers
    // when the long-lived worker + dashboard both connect and the heartbeat
    // keeps connections alive. Cap hard, close idle connections ourselves,
    // and fail fast under saturation.
    const pool = new Pool({
      connectionString: databaseUrl,
      max: 3,                    // Hard cap: never exceed pooler limit
      min: 1,                    // Keep 1 warm connection
      idleTimeoutMillis: 10000,  // Close idle connections after 10s
      connectionTimeoutMillis: 5000, // Fail fast if pooler is saturated
      allowExitOnIdle: true,     // Let the process exit if pool is idle
    });
